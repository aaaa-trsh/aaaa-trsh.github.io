document.getElementById("info").innerHTML = "(not quite) rule 30 feeding a game of life";
...
BASE_COLOR = d3.interpolatePlasma(0);
    ...
    for (var i = 0; i < numPixels; i++) {
        ...
        if (pixels[i*4+0] != 255){
            if (pixels[i*4+0] > 3) {
                ...
                //     b: pixels[i*4+0] * .9
                // };
                color = d3.interpolatePlasma(Math.pow((pixels[i*4+0]*.9)/255, 1.7))
                if (color === BASE_COLOR) {
                    color = {
                        r: 0,
                        g: 0,
