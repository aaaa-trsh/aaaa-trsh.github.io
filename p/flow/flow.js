...
function init() {
    ...
    var count = Math.min(window.innerHeight, window.innerWidth) / .9;
...
function drawParticle(x, y, i) {
    ...
    var addX = lerp(attractor[0]*-2, noiseAngle[0], attracting ? clamp(mix, 0, 1) : 1)*cellSpeed * noiseValue * 3;//*(clamp(mix, .3, 2))
    var addY = lerp(attractor[1]*-2, noiseAngle[1], attracting ? clamp(mix, 0, 1) : 1)*cellSpeed * noiseValue * 3;//*(clamp(mix, .3, 2))
